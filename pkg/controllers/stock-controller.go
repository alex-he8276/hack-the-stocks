...
func retrieveTweets(ticker string, date time.Time, startId string) TwitterResult {
	...
	if(startId != "") {
		url = fmt.Sprintf("%s&since_id=%s",url,startId)
	}
...
func GetStockSentiment(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < numDays; i++ {
		...
		//used to store first valid tweet ID and text
		firstTweet := Tweet{}
		...
		go func(i int) {
			...
			if !exists {
				...
				fmt.Println("Ticker: ", ticker, " | date: ", date)

				// get text from twitter response
				tweets := make([]string, 0)

				//Keep fetching tweets until we get two times number of tweets per request or we run out of tweet to fetch
				for  len(tweets) <= numTweetsPerDay*2 {
					// Retrieve tweets from twitter api
					twitterResponse := retrieveTweets(ticker, date, startTweetID)
					if(len(twitterResponse.Tweets) > 0){
						startTweetID = twitterResponse.Tweets[0].ID
					}
					...
					//If we get to a point where we dont return enough tweets, we return immediately
					if(len(twitterResponse.Tweets) < numTweetsPerDay) {
				...
				fmt.Println("Curling", len(tweets), " tweets ", i)
				...
				if( len(tweets) > 0){
					...
					offset := 0
					for offset < len(tweets) {

						

						offsetUpper := offset + 29
						//Ensure upper bound of offset is less than or equal to number of tweets
						if(offsetUpper >= len(tweets)) {
							offsetUpper = len(tweets)
						}
						fmt.Println("segmenting %d to %d tweets", offset, offsetUpper)

						//Get 30 tweet slice from tweet array
						sliceTweets := tweets[offset: offsetUpper ]
						
						cohereResponse = classify(co, ticker, date, sliceTweets)
						for _, classification := range cohereResponse.Classifications {
							if classification.Prediction == "1" {
								sentiment += 1
							}
						}
						//Get example classification
						if(offset == 0){
							for _, classf := range cohereResponse.Classifications{
								if classf.Input == firstTweet.Text {
									if classf.Prediction == "1" {
					...
					sentiment = sentiment * 100 / float64(len(tweets))
			...
			//Only append result if its defined
			if(stock != nil){
